...
2. A comma-separated list of operations. All tests will be executed by default if this option is omitted. Currently supported operations:
   ...
   | `context-showcase::deploy`                 | Deploy AccountCreator contract    | Deploys a contract.                                                                 |
   | `context-showcase::precision`              | Triggers precision dependant code | Gas usage on Hedera may differ when different opcodes are executed than on Sepolia. |
