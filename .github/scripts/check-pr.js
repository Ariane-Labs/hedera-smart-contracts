...
async function getPRDetails(prNumber) {
  ...
  try {
    ...
    if (error.response && error.response.status === 404) {
      console.log(
        `PR #${prNumber} not found in repository ${owner}/${repo}, skipping...`
      );
      return null;
    } else {
      throw error;
...
async function getIssueDetails(issueOwner, issueRepo, issueNumber) {
  try {
    ...
    if (error.response && error.response.status === 404) {
      console.log(
        `Issue #${issueNumber} not found in repository ${issueOwner}/${issueRepo}, skipping...`
      );
      return null;
    } else {
      throw error;
...
function isDependabotOrSnykPR(pr) {
  return (
    ...
    pr.user.login === 'swirlds-automation'
  );
}
...
async function processIssueReferencesInText(text) {
  ...
  if (issueReferences.length > 0) {
    ...
    if (!hasValidIssueReference) {
      throw new Error(
        'The PR description must reference at least one issue from the current repository.'
      );
    } else {
...
async function processReferencedPR(prRef, contributors) {
  ...
  const isContributor = contributors.some(
    (contributor) => contributor.login === authorLogin
  );
  ...
  if (refIssueReferences.length === 0) {
    console.log(
      `No associated issues found in PR #${prRef.prNumber} description.`
    );
  } else {
...
async function run() {
  try {
    ...
    if (isDependabotOrSnykPR(pr)) {
      console.log(
        'Dependabot or snyk PR detected. Skipping issue reference requirement.'
      );
      return;
    } else {
